...
def chat_youtube_log_get(user):
    ...
    if validLogKeys[logMethod]['key'] is None:
        return jsonify({
            'error': "invalid_request",
...
def _rankChatters(videoID, resultsPerPage, pageNum,
    ...
    if incMods == 'only':
        query = query.filter(MessageLog.is_mod)
    elif incMods == 'exclude':
        query = query.filter(not MessageLog.is_mod)

    if incMembers == 'only':
        query = query.filter(MessageLog.is_sponsor)
    elif incMembers == 'exclude':
        query = query.filter(not MessageLog.is_sponsor)
...
def _chatLogByMsgID(videoID, msgID, perPage, pageNum):
    ...
    for r in reversed(prevMsgResult):  # query will come in reversed order
        chatLog.append({
            ...
            'timestamp': r.timestamp.strftime("%m/%d/%y, %H:%M:%S"),
            })
...
def _chatLogByAuthorID(videoID, authorID, perPage, pageNum):
    ...
    for result in r:
        userChatMessage = {
                ...
                'timestamp': result.timestamp.strftime("%m/%d/%y, %H:%M:%S"),
                }
